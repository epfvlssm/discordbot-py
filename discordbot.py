...
async def on_message(message):
    ...
    elif message.content.startswith('%황금상자'): # 메세지가 %황금상자 로 시작하는 경우
        dice_result = str(random.choice(['먼지', '가챠 1회 무료 이용권', '슬롯머신 1회 무료 이용권(1텔로 적용)', '복권 1회 무료 이용권', '안드레이아 기념 주화', '폼폼열쇠'])) # 1~100 랜덤 선택 (1d100)
        await message.channel.send(f'황금빛 상자를 열자…… 안에 들어있던 것은…! <{dice_result}>입니다.', reference=message) # 답장 o        
         ...
         await message.channel.send(embed=embed, reference = message)
          ...
          if SasaNum == 0:
            ...
            Text = '퍼레이드길'
            Text2 = '멋진 퍼레이드가 진행되는 넓은 길이야. 운이 좋아 퍼레이드길에 도착하자마자 멋진 퍼레이드가 막 시작하고 있었어. 퍼레이드를 처음부터 볼 수 있었네. 시작 시간은 폼폼 마음대로라니 정말인가봐. 재밌었네! 퍼레이드를 구경한 기념으로 **8퍼레이드를 구경하는 폼폼 아크릴 키링***을 얻을 수 있었어.'
